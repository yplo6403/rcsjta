...
public class CmsSchedulerTest extends AndroidTestCase {
    ...
    public void testInitScheduler() throws InterruptedException {
        ...
        Assert.assertEquals(0,
                mOperationListener.getExecutions(CmsOperation.SYNC_FOR_DATA_CONNECTION));
        mScheduler.start();
        ...
        Assert.assertFalse(mScheduler.mSyncRequestHandler
                .hasMessages(CmsOperation.SYNC_FOR_DATA_CONNECTION.toInt()));
        Assert.assertTrue(mScheduler.mSyncRequestHandler.hasMessages(CmsOperation.SYNC_PERIODIC
                .toInt()));
        ...
        Assert.assertFalse(mScheduler.mSyncRequestHandler.hasMessages(CmsOperation.SYNC_PERIODIC
                .toInt()));
        Thread.sleep(executionDuration + 200, 0);
        Assert.assertEquals(1,
                mOperationListener.getExecutions(CmsOperation.SYNC_FOR_USER_ACTIVITY));
        Assert.assertTrue(mScheduler.mSyncRequestHandler.hasMessages(CmsOperation.SYNC_PERIODIC
                .toInt()));
