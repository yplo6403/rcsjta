...
public class ImapLogTest extends AndroidTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        mFolders = new FolderData[] {
                new FolderData("folder1", 1, 123, 1), new FolderData("folder2", 1, 1234, 1),
                new FolderData("folder3", 1, 12345, 1)
        };

        mMessages = new MessageData[] {
                new MessageData(mFolders[0].getName(), 1, ReadStatus.UNREAD,
                        DeleteStatus.NOT_DELETED, PushStatus.PUSHED, MessageType.SMS, "messageId1",
                        null),
                new MessageData(mFolders[1].getName(), 1, ReadStatus.UNREAD,
                        DeleteStatus.NOT_DELETED, PushStatus.PUSHED, MessageType.SMS, "messageId1",
                        null),
                new MessageData(mFolders[1].getName(), 2, ReadStatus.UNREAD,
                        DeleteStatus.NOT_DELETED, PushStatus.PUSHED, MessageType.SMS, "messageId2",
                        null),
                new MessageData(mFolders[2].getName(), 1, ReadStatus.UNREAD,
                        DeleteStatus.NOT_DELETED, PushStatus.PUSHED, MessageType.SMS, "messageId1",
                        null),
                new MessageData(mFolders[2].getName(), 2, ReadStatus.UNREAD,
                        DeleteStatus.NOT_DELETED, PushStatus.PUSHED, MessageType.SMS, "messageId2",
                        null),
                new MessageData(mFolders[2].getName(), 3, ReadStatus.UNREAD,
    ...
    public void testUpdateFolder() {
        ...
        mImapLog.addFolder(new FolderData(mFolders[0].getName(), 0, 0, 0));
    ...
    public void testGetMessages() {
        ...
        for (int i = 0; i < mMessages.length; i++) {
            assertEquals(mMessages[i],
                    mImapLog.getMessage(mMessages[i].getFolder(), mMessages[i].getUid()));
        }

        for (int i = 0; i < mMessages.length; i++) {
            assertEquals(
                    mMessages[i],
                    mImapLogEnvIntegration.getMessage(mMessages[i].getFolder(),
        ...
        for (int i = 0; i < mMessages.length; i++) {
            assertEquals(mMessages[i].getUid(),
                    mImapLog.getUidForXmsMessage(mMessages[i].getMessageId()));
        }
    ...
    public void testUpdateMessage() {
        ...
        mImapLog.addMessage(new MessageData(mMessages[0].getFolder(), 1, ReadStatus.READ,
