...
public class CmsService extends ImsService {
    ...
    private ReceiveSmsEventSent mReceiveSmsEventDelivered;
    ...
    public void sendSms(ContactId contact, String text) {
        ...
        PendingIntent mSentPendingIntent = PendingIntent.getBroadcast(mContext, 0, smsSentIntent, 0);
        Intent smsDeliveredIntent = new Intent(SMS_DELIVERED_ACTION);
        smsDeliveredIntent.putExtra(KEY_TRANSACTION_ID, transactionId);
        PendingIntent mDeliveredPendingIntent = PendingIntent.getBroadcast(mContext, 0, smsDeliveredIntent, 0);
        ...
        if (parts.size() > 1) {
            ...
            if (sLogger.isActivated()) {
                sLogger.debug("Sending split message of " + text.length() + " characters into " + parts.size() + " parts.");
            }
            smsManager.sendMultipartTextMessage(contact.toString(), null, parts, sentPIs, deliveredPIs);
        } else {
            if (sLogger.isActivated()) {
                sLogger.debug("sendTextMessage to " + contact + " text='" + text + "' with transactionId=" + transactionId);
            }
            smsManager.sendTextMessage(contact.toString(), null, text, mSentPendingIntent, mDeliveredPendingIntent);
        }
    ...
    public void sendMms(ContactId contact, String text, ArrayList<Uri> files) {
    ...
    private class ReceiveSmsEventSent extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (partIndex != -1) {
                if (sLogger.isActivated()) {
                    sLogger.debug("ReceiveSmsEventSent for transaction ID=" + transId + " for part number " + partIndex);
                }
            } else {
                if (sLogger.isActivated()) {
            ...
            new Thread() {
                public void run() {
                    try {
                         ...
                         */
                        sLogger.error("Failed to process delivery event!", e);
                    }
                }
            }.start();
        }
    ...
    private class ReceiveSmsEventDelivered extends ReceiveSmsEventSent {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (partIndex != -1) {
                if (sLogger.isActivated()) {
                    sLogger.debug("ReceiveSmsEventDelivered for transaction ID=" + transId + " for part number " + partIndex);
                }
            } else {
                if (sLogger.isActivated()) {
