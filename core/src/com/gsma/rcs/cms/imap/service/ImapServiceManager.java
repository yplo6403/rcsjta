...
public class ImapServiceManager {
    ...
    private static final Logger sLogger = Logger
            .getLogger(ImapServiceManager.class.getSimpleName());
     ...
     */
    public static BasicImapService getService(RcsSettings settings)
            throws ImapServiceNotAvailableException {
        ...
        if (!sSemaphore.tryAcquire()) {
            ...
            throw new ImapServiceNotAvailableException(
                    "A imap sync is already in progress. Can not start a new one yet.");
        }
        if (sLogger.isActivated()) {
        ...
        service.setAuthenticationDetails(settings.getCmsUserLogin(), settings.getCmsUserPwd(),
                null, null, false);
        return service;
    }
