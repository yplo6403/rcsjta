...
public class DefaultImapService implements ImapService {
    ...
    protected ImapNamespace mPersonalNamespace;// , othersNamespace, publicNamespace;
    ...
    protected int getUidPlus(String response) {
        if (uidPlusSupported()) {
    ...
    protected synchronized String ioReadLine() throws IOException, ImapException {
        autoConnect();
        return mIoService.readLine();
    }

    protected synchronized void ioWriteln(String payload) throws IOException, ImapException {
    ...
    protected boolean checkResponseOk(String line) {
        return line.startsWith(mCurrentTag + " OK");
    }

    protected boolean isTagged(String line) {
