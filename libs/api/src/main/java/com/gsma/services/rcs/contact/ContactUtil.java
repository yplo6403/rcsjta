...
public class ContactUtil {
    ...
    public ContactId formatContact(String contact) throws RcsPermissionDeniedException {
        ...
        if (TextUtils.isEmpty(strippedContact)) {
            throw new IllegalArgumentException("Contact '" + contact
                    + "' has invalid characters or is too long!");
        }
        ...
        throw new IllegalArgumentException("Local phone number '" + strippedContact
                + "' should be prefixed with Country Area Code (" + mCountryAreaCode + ")");
    }
    ...
    private void tryToDetermineAndCacheCountryAndAreaCodes() throws RcsPermissionDeniedException {
        synchronized (ContactUtil.class) {
            ...
            if (countryCodeInfo == null) {
                throw new RcsPermissionDeniedException("Failed to get mobile country code ("
                        + config.mcc + ")!");
            }
    ...
    public Uri getVCard(Uri contactUri) throws RcsGenericException {
        ...
        try {
            ...
            String fileName = Environment.getExternalStorageDirectory().toString() + File.separator
                    + name + ".vcf";
