...
public final class MmsEncoder
{
	...
	*--------------------------*/
	...
	private MmsEncodedMessage message;
	 ...
	 */
	public byte[] encode() throws MmsException
	{
		try {
			...
			// Type
			if (this.message.getType() == null || this.message.getType().length() == 0) {
				throw new MmsFormatException("No MMS type !");
			}
			...
			// [WARNING] MANDATORY FIELD (TODO)
			// Transaction ID
			if (this.message.getTransactionId() == null || this.message.getTransactionId().length() == 0) {
				throw new MmsFormatException("No MMS transaction Id !");
			}
			...
			// Date
			if (this.message.getDate() == -1L || this.message.getDate() < 0) {
				throw new MmsFormatException("No MMS date !");
			}
			...
			// [WARNING] MANDATORY FIELD (TODO)
			// From
			if (this.message.getFrom() == null || this.message.getFrom().length() == 0) {
				throw new MmsFormatException("No MMS transaction Id !");
			}
			...
			// To
			if (this.message.getTo() == null || this.message.getTo().size() == 0) {
				throw new MmsFormatException("No MMS dest !");
			}
			for (String to : this.message.getTo()) {
				outBuffer.write(TO);
			...
			// [WARNING] MANDATORY FIELD (TODO)
			// Content type
			if (this.message.getContentType() == null || this.message.getContentType().length() == 0) {
				throw new MmsFormatException("No MMS content type !");
			}
			outBuffer.write(CONTENT_TYPE);
			outBuffer.write(MMS_CONTENT_TYPES.get(this.message.getContentType()) + 0x80);

			// Write parts
			if (this.message.getParts() == null || this.message.getParts().size() == 0) {
				throw new MmsFormatException("No MMS part !");
			}
			...
			throw new MmsIOException(ioe.getMessage(),ioe);
		}
	}
	 ...
	 */
	private byte[] encodePart(MmsEncodedPart part)
	{
		// Define content type
		int contentType = MMS_CONTENT_TYPES.get( part.getContentType() ) == null ? 0 : MMS_CONTENT_TYPES.get( part.getContentType() );
